...
基于 HTML demo 的 JavaScript 实现，以下是关键的交互逻辑细节：
...
// 焦点管理器
struct FocusManager {
    current_area: FocusArea,
    previous_area: Option<FocusArea>,
...
pub struct RefreshManager {
    ...
    refresh_interval: Duration,  // 默认 30 秒
...
impl RefreshManager {
    pub async fn refresh_current_view(&mut self) -> Result<()> {
        self.sync_status = SyncStatus::Refreshing;
        // 执行刷新逻辑
        match self.current_view {
            ViewType::Branches => self.refresh_branches().await?,
            ViewType::Status => self.refresh_git_status().await?,
            // ... 其他视图
        }
...
**键盘事件处理链：**
1. **模态框层** - 最高优先级，拦截所有键盘事件
2. **命令模式** - 次高优先级，处理命令输入
3. **搜索模式** - 处理搜索相关快捷键
4. **视图特定** - 各视图的专属快捷键
5. **全局快捷键** - 最低优先级，如退出、刷新等

### Cherry Pick 和高级 Git 操作

**Cherry Pick 操作界面：**
```
┌─ Cherry Pick Commit ───────────────────────────────────────┐
│ Picking: 18c4a3a - fix(ui): 修复侧边栏布局问题             │
│                                                            │
│ Target Branch: [main ▼]                                    │
│                                                            │
│ Options:                                                   │
...
│                                                            │
│ Available Actions:                                         │
│ • [Stage All] - Add selected files to staging area         │
│ • [Discard All] - Discard changes in selected files        │
│ • [Delete All] - Remove selected files                     │
│                                                            │
│ [Apply] [Clear Selection] [Cancel]                         │
└────────────────────────────────────────────────────────────┘
```
...
pub struct SessionState {
    ...
    selected_indices: HashMap<ViewType, usize>,
...
pub fn check_terminal_size(width: u16, height: u16) -> Result<()> {
    if width < MIN_TERMINAL_WIDTH || height < MIN_TERMINAL_HEIGHT {
        return Err(anyhow!(
            "Terminal too small. Minimum size: {}x{}",
            MIN_TERMINAL_WIDTH,
            MIN_TERMINAL_HEIGHT
        ));
    }
...
基于 HTML demo 的模态对话框设计，我们需要实现以下模态界面系统：
...
基于 `tui-demo.html` 的实现，以下是需要补充的具体功能设计：
...
```
┌─ Sync Status ──────────────────────────────────────────────┐
│ 🔄 Refreshing... | Status: Ready | Auto-refresh: ON       │
└────────────────────────────────────────────────────────────┘
```

**自动刷新功能：**
- 显示刷新动画图标
- 状态文本实时更新（Ready/Refreshing...）
- 自动刷新开关显示
...
│ ✓ src/tui/layout/manager.rs              Modified         │
│ ✓ src/components/sidebar.rs              Modified         │
│ ✓ docs/new-layout-design.md              Added            │
│                                                            │
│ Total: 3 files, +68 -17 lines                             │
└────────────────────────────────────────────────────────────┘
```
...
pub struct SelectionManager {
    ...
    // AI 建议选择
    selected_ai_suggestion: Option<usize>,
}
```
...
│ └────────────────────────────────────────────────────────┘ │
│                                                            │
│ [Ctrl+S] Start Rebase | [Esc] Cancel                       │
└────────────────────────────────────────────────────────────┘
```
...
│ AI 增强建议:                                                │
│ ┌──────────────────────────────────────────────────────┐   │
│ │ [feat] fix(ui): 重构侧边栏布局系统                     │   │
│ │        解决信息层次混乱问题                            │   │
│ │                                                        │   │
│ │ [fix]  fix(ui): 修复侧边栏布局问题                     │   │
│ │        UI 修复类型格式                                 │   │
│ │                                                        │   │
│ │ [refactor] refactor(layout): 优化TUI布局管理系统       │   │
│ │            核心重构格式                                │   │
│ └──────────────────────────────────────────────────────┘   │
│                                                            │
│ [使用选中] [保持原样] [取消]                                │
...
```
┌─ Quick Actions ────────────────────────────────────────────┐
│ [r] Reword  [e] Edit  [s] Squash  [Enter] Show Diff       │
│ [Ctrl+E] Interactive Rebase                                │
└────────────────────────────────────────────────────────────┘
```
...
```
┌─ Quick Actions ────────────────────────────────────────────┐
│ [Commit] [AI Commit] [Add All] [Reset] [Discard]          │
│ [🔄 Refresh]                                               │
└────────────────────────────────────────────────────────────┘
```
...
```
┌────────────────────────────────────────────────────────────┐
│ [Enter] Toggle | [Space] Stage/Unstage | [d] Diff         │
│ [c] Commit | [↑↓] Navigate                                │
└────────────────────────────────────────────────────────────┘
```
