...
mod tests {
    ...
    fn test_env_vars_batch_loading() {
        ...
        env::set_var("AI_COMMIT_SILICONFLOW_URL", "https://custom.siliconflow.com");
        
        let env_vars = EnvVars::load();
        
        assert_eq!(env_vars.provider, Some("siliconflow".to_string()));
        assert_eq!(env_vars.model, Some("qwen-plus".to_string()));
        assert_eq!(env_vars.siliconflow_api_key, Some("test-siliconflow-key".to_string()));
        assert_eq!(env_vars.siliconflow_url, Some("https://custom.siliconflow.com".to_string()));
    ...
    fn test_all_providers_validation() {
        ...
        for (provider, api_key, should_pass) in test_cases {
            ...
            if should_pass {
                assert!(result.is_ok(), "Provider '{}' with key {:?} should pass validation", provider, api_key);
            } else {
                assert!(result.is_err(), "Provider '{}' with key {:?} should fail validation", provider, api_key);
            }
        }
    ...
    fn test_config_default_values() {
        ...
        assert_eq!(config.provider, "ollama");
        assert_eq!(config.model, "mistral");
        assert_eq!(config.deepseek_url, "https://api.deepseek.com/v1/chat/completions");
        assert_eq!(config.ollama_url, "http://localhost:11434/api/generate");
        assert_eq!(config.siliconflow_url, "https://api.siliconflow.cn/v1/chat/completions");
        assert!(config.deepseek_api_key.is_none());
        assert!(config.siliconflow_api_key.is_none());
    ...
    fn test_partial_env_vars() {
        ...
        assert_eq!(config.deepseek_url, "https://api.deepseek.com/v1/chat/completions"); // 默认值
