...
pub async fn clear_other_worktrees() -> anyhow::Result<usize> {
    ...
    for worktree in worktrees {
        ...
        match remove_worktree(&worktree.path.to_string_lossy()).await {
            ...
            Err(e) => {
                eprintln!("Failed to remove worktree {}: {}", worktree.path.display(), e);
            }
        }
    }
...
mod tests {
    ...
    fn test_worktree_path_matching() {
        ...
        // 测试通过路径匹配
        let match_by_path = worktrees.iter().find(|w| {
            w.path.to_string_lossy().contains("feature-test")
        });
        assert!(match_by_path.is_some());
        assert_eq!(match_by_path.unwrap().branch, "feature/test");
        
        // 测试通过分支名匹配
        let match_by_branch = worktrees.iter().find(|w| {
            w.branch.contains("main")
        });
    ...
    fn test_worktree_filtering_for_clear() {
        ...
        let to_remove: Vec<_> = worktrees.iter().filter(|w| {
            w.path != current_dir && !w.is_bare
        }).collect();
    ...
    fn test_worktree_name_patterns() {
        ...
        for (search_term, branch_name, should_match) in test_cases {
            ...
            assert_eq!(matches, should_match, 
                      "Failed for search_term: {}, branch: {}", search_term, branch_name);
        }
    }
    ...
    fn test_clear_worktree_count_calculation() {
        ...
        let test_scenarios = vec![
            // 场景1: 只有当前worktree
            (vec![
                WorktreeInfo::new(current_dir.clone(), "main".to_string(), "abc".to_string(), false, false)
            ], 0),
            
            // 场景2: 当前 + 1个其他
            (vec![
                WorktreeInfo::new(current_dir.clone(), "main".to_string(), "abc".to_string(), false, false),
                WorktreeInfo::new(PathBuf::from("/repo/other"), "feature".to_string(), "def".to_string(), false, false)
            ], 1),
            
            // 场景3: 当前 + 1个bare + 2个其他
            (vec![
                WorktreeInfo::new(current_dir.clone(), "main".to_string(), "abc".to_string(), false, false),
                WorktreeInfo::new(PathBuf::from("/repo/bare"), "bare".to_string(), "def".to_string(), true, false),
                WorktreeInfo::new(PathBuf::from("/repo/other1"), "f1".to_string(), "ghi".to_string(), false, false),
                WorktreeInfo::new(PathBuf::from("/repo/other2"), "f2".to_string(), "jkl".to_string(), false, false)
            ], 2),
        ];
        
        for (worktrees, expected_count) in test_scenarios {
            let removable_count = worktrees.iter().filter(|w| {
                w.path != current_dir && !w.is_bare
            }).count();
    ...
    fn test_path_matching_edge_cases() {
        ...
        for term in search_terms {
            let found = worktrees.iter().find(|w| {
                w.path.to_string_lossy().contains(term) || w.branch.contains(term)
            });
