    ...
    fn load_default_strings(&mut self) {
        ...
        messages.insert("commit_message_generated".to_string(), {
            ...
            m.insert(
                ...
                "AI生成commit消息耗时".to_string(),
            );
            m.insert(
                Language::TraditionalChinese,
                "AI生成commit消息耗時".to_string(),
            );
            m.insert(
                Language::English,
...
mod tests {
    ...
    fn test_i18n_get_messages() {
        ...
        assert_eq!(
            i18n.get("commit_message_generated"),
            "AI generated commit message duration"
        );
    }
    ...
    fn test_i18n_message_coverage() {
        ...
        for key in expected_keys {
            for lang in &languages {
                ...
                assert!(
                    !message.is_empty(),
                    "Message for key '{}' in language {:?} should not be empty",
                    key,
                    lang
                );
            }
    ...
    fn test_language_case_insensitive() {
        ...
        for (lower, upper) in test_cases {
            ...
            assert_eq!(
                lang_lower, lang_upper,
                "Case sensitivity test failed for '{}' vs '{}'",
                lower, upper
            );
        }
    }
    ...
    fn test_message_content_validity() {
        ...
        for (key, expected_substrings) in test_cases {
            ...
            assert!(
                found_expected,
                "Key '{}' should contain at least one of {:?} in some language",
                key, expected_substrings
            );
        }
    }
} // 国际化修改
