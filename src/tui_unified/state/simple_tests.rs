...
mod simple_state_tests {
    ...
    use crate::tui_unified::state::{
        AppState, GitRepoState, LayoutState, NotificationLevel, SimpleStatePersistence, ViewType,
    };
    ...
    async fn test_selection_management() {
        ...
        assert_eq!(
            app_state.selected_items.selected_commit,
        ...
        assert_eq!(
            app_state.selected_items.selected_branch,
            Some("main".to_string())
        );
        ...
        assert_eq!(
            app_state.get_current_selection(),
            Some("abc123".to_string())
        );
