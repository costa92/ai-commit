use ai_commit::{cli::args::Args, commands, config::Config, ui};
use clap::Parser;
...
mod new_features_integration_tests {
    ...
    async fn test_commit_commands_with_skip_confirm() {
        ...
        match result {
            ...
            Err(e) => {
                println!(
                    "Commit command failed as expected in test environment: {}",
                    e
                );
                // 验证错误消息包含预期的内容
                ...
                assert!(
                    ...
                    "Error should be related to git operations or AI service: {}",
                    error_str
                );
            }
        }
        ...
        match result {
            ...
            Err(e) => {
                ...
                assert!(
                    ...
                    "Error should be related to git operations or AI services: {}",
                    error_str
                );
            }
        }
    ...
    fn test_comprehensive_argument_validation() {
        ...
        for (args_vec, expected_skip_confirm, expected_force_push) in test_cases {
            ...
            assert_eq!(
                args.skip_confirm, expected_skip_confirm,
                "Failed for args: {:?}",
                args_vec
            );
            assert_eq!(
                args.force_push, expected_force_push,
                "Failed for args: {:?}",
                args_vec
            );
        }
    ...
    fn test_feature_interaction_matrix() {
        ...
        let base_features: Vec<(&str, BoolChecker)> = vec![
            ...
            (
                "--push-branches",
                Box::new(|args: &Args| args.push_branches),
            ),
        ];
        ...
        for (base_flag, base_checker) in &base_features {
            for (new_flag, new_checker) in &new_features {
                let args = Args::try_parse_from(["ai-commit", base_flag, new_flag]).unwrap();

                assert!(
                    base_checker(&args),
                    "Base feature {} should be enabled",
                    base_flag
                );
                assert!(
                    new_checker(&args),
...
mod performance_tests {
    ...
    fn test_argument_parsing_performance() {
        ...
        assert!(
            ...
            "Argument parsing took too long: {:?}",
            elapsed
        );
    }
    ...
    fn test_ui_enum_performance() {
        ...
        let elapsed = start.elapsed();
        println!(
            "Created and matched 10000 ConfirmResult enums in {:?}",
            elapsed
        );
...
mod edge_case_tests {
    ...
    fn test_empty_and_special_inputs() {
        ...
        // 空的提供商和模型
        let args =
            Args::try_parse_from(["ai-commit", "--provider", "", "--model", "", "--yes"]).unwrap();
    ...
    fn test_very_long_inputs() {
        ...
        let args =
            Args::try_parse_from(["ai-commit", "--tag-note", &long_tag_note, "--yes"]).unwrap();
