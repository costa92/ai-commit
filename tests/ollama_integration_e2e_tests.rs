...
async fn test_ollama_connection(model: &str) -> Result<String, Box<dyn std::error::Error>> {
    ...
    let json: Value = response.json().await?;
    let generated_text = json["response"].as_str().unwrap_or("").to_string();
...
fn test_e2e_ollama_provider_info_completeness() {
    ...
    let ollama = ProviderRegistry::get_provider("ollama").expect("应该能找到 ollama 提供商");
    ...
    assert_eq!(
        ollama.api_format,
        ai_commit::config::providers::ApiFormat::Ollama
    );
    ...
    assert!(
        ollama.supported_models.contains(&ollama.default_model),
        "默认模型应该在支持的模型列表中"
    );
...
fn test_e2e_ollama_config_backwards_compatibility() {
    ...
    assert!(
        ...
        "deepseek_api_key() 应该返回 None"
    );
    assert!(
        !config.deepseek_url().is_empty(),
        "deepseek_url() 应该返回有效值"
    );
