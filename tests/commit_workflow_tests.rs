...
mod commit_workflow_tests {
    use ai_commit::{cli::args::Args, commands, config::Config, git, ui};
    use clap::Parser;
    use tokio;
    ...
    async fn test_commit_command_with_skip_confirm() {
        ...
        match result {
            ...
            Err(e) => {
                ...
                let contains_expected = expected_keywords
                    .iter()
    ...
    async fn test_git_force_push_functionality() {
        ...
        match result {
            ...
            Err(e) => {
                ...
                let contains_expected = expected_errors
                    .iter()
                    .any(|expected| error_msg.contains(expected));

                assert!(
                    contains_expected,
                    "强制推送错误应该包含预期的git操作错误: {}",
                    error_msg
                );
            }
        }
    ...
    fn test_workflow_parameter_validation() {
        ...
        let valid_combinations = vec![
            ...
            (
                vec!["ai-commit", "--yes", "--push", "--force-push"],
                true,
                true,
                true,
            ),
        ...
        for (args_vec, expected_skip, expected_push, expected_force) in valid_combinations {
            ...
            assert_eq!(
                args.skip_confirm, expected_skip,
                "skip_confirm 应该匹配，参数: {:?}",
                args_vec
            );
            assert_eq!(
            ...
            assert_eq!(
                args.force_push, expected_force,
                "force_push 应该匹配，参数: {:?}",
                args_vec
            );
        }
    ...
    fn test_workflow_integration_scenarios() {
        ...
        // 场景 1: 快速提交（跳过确认，直接推送）
        let quick_commit = Args::try_parse_from(["ai-commit", "--yes", "--push"]).unwrap();
        assert!(quick_commit.skip_confirm);
        assert!(quick_commit.push);
        assert!(!quick_commit.force_push);

        // 场景 2: 安全提交（需要确认，解决冲突）
        let safe_commit = Args::try_parse_from(["ai-commit", "--push", "--force-push"]).unwrap();
