...
mod force_push_tests {
    use ai_commit::{cli::args::Args, config::Config, git};
    use clap::Parser;
    use tokio;
    ...
    async fn test_git_force_push_basic() {
        ...
        match result {
            ...
            Err(e) => {
                ...
                let contains_git_error = git_errors
                    .iter()
    ...
    fn test_force_push_parameter_combinations() {
        ...
        let test_cases = vec![
            ...
            (
                vec!["ai-commit", "--force-push", "--push"],
                true,
                true,
                "force-push + push",
            ),
            (
                vec!["ai-commit", "--push", "--force-push"],
                true,
                true,
                "push + force-push（顺序不同）",
            ),
        ];
        ...
        for (args_vec, expected_force_push, expected_push, description) in test_cases {
            ...
            assert_eq!(
                ...
                "force_push不匹配，场景: {} 参数: {:?}",
                description, args_vec
            );
            assert_eq!(
                args.push, expected_push,
                "push不匹配，场景: {} 参数: {:?}",
                description, args_vec
            );
        }
    ...
    fn test_parameter_precedence_and_conflicts() {
        ...
        // 测试短参数和长参数的等价性
        let long_form =
            Args::try_parse_from(["ai-commit", "--yes", "--push", "--force-push"]).unwrap();

        let short_form =
            Args::try_parse_from(["ai-commit", "-y", "--push", "--force-push"]).unwrap();
    ...
    fn test_edge_cases_and_error_conditions() {
        ...
        let long_args =
            Args::try_parse_from(["ai-commit", "--tag-note", &very_long_note, "--yes"]).unwrap();
        ...
        let special_args =
            Args::try_parse_from(["ai-commit", "--tag-note", special_chars_note, "--yes"]).unwrap();
    ...
    fn test_memory_usage_patterns() {
        ...
        let complex_args = Args::try_parse_from([
            "ai-commit",
            "--provider",
            "very-long-provider-name-for-testing",
            "--model",
            "very-long-model-name-for-testing",
            "--tag-note",
            &"很长的标签说明".repeat(100),
            "--yes",
            "--force-push",
