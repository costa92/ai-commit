...
use ai_commit::config::providers::{ApiFormat, ProviderInfo, ProviderRegistry};
...
fn test_e2e_provider_registry_basic_functionality() {
    ...
    assert!(
        providers.contains(&"siliconflow"),
        "应该包含 siliconflow 提供商"
    );
...
fn test_e2e_provider_validation_workflow() {
    ...
    assert!(
        ...
        "没有 API Key 的 Deepseek 配置应该验证失败"
    );
...
fn test_e2e_provider_info_completeness() {
    ...
    for (name, provider) in all_providers.iter() {
        ...
        assert!(
            api_key_var.contains("API_KEY"),
            "API Key 环境变量应该包含 API_KEY"
        );
...
fn test_e2e_provider_error_messages() {
    ...
    assert!(
        error_msg.contains("AI_COMMIT_DEEPSEEK_API_KEY"),
        "错误消息应该包含环境变量名"
    );
...
fn test_e2e_all_providers_basic_config() {
    ...
    for provider_name in all_providers {
        ...
        if provider_info.requires_api_key {
            assert!(
                config.current_api_key().is_some(),
                "需要 API Key 的提供商应该有 API Key"
            );
        } else {
            assert!(
                config.current_api_key().is_none(),
                "不需要 API Key 的提供商不应该有 API Key"
            );
        }
